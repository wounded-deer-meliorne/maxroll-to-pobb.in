// content_buildguide.js — PathToPobb.in (Firefox)
...
(function () {
  ...
  function startDetection() {
    ...
    const detect = setInterval(() => {
      ...
      if (url) {
        ...
        browser.runtime.sendMessage({ type: 'REDIRECT_TAB', url });
        return;
      }
      if (++attempts >= MAX) {
        clearInterval(detect);
        detecting = false;
        showError();
        console.warn('[PathToPobb.in] Build guide: gave up after 30s on', window.location.href);
      }
    }, 500);
  }
  ...
  function watchNavigation() {
    ...
    if (currentUrl !== lastHandledUrl && isBuildGuidePage(currentUrl)) {
      console.log('[PathToPobb.in] Build guide: navigation to', currentUrl);
