...
(function () {
  ...
  const overlay = document.createElement('div');e
  ...
  function tryFind(attempt = 0) {
    ...
    if (url) {
      console.log('[maxroll-to-pobb] Found PoB URL on build guide:', url);
    ...
    if (attempt < MAX_ATTEMPTS) {
      ...
      overlay.remove();
      console.warn('[maxroll-to-pobb] Could not find a.poe-planner-pob-link after 20s.');
    }
  }
  ...
  const observer = new MutationObserver(() => {
    ...
    if (url) {
      observer.disconnect();
      console.log('[maxroll-to-pobb] MutationObserver found PoB URL:', url);
      window.location.href = url;
    }
  });
